#ifndef MXNET_OPERATOR_CONTRIB_TRT_INL_H_
#define MXNET_OPERATOR_CONTRIB_TRT_INL_H_
/*
...
struct TRTParam : public dmlc::Parameter<TRTParam> {
  ...
  trt::InferenceMap_t output_map;
  ::onnx::ModelProto onnx_pb_graph;

  TRTParam() {}

  TRTParam(const ::onnx::ModelProto& onnx_graph,
           const trt::InferenceMap_t& input_map,
           const trt::NameToIdx_t& output_map) {
...
struct TRTEngineParam {
  ...
  std::vector<std::pair<uint32_t, trt::TypeIO> > binding_map;
};
...
void ConvertOutput(op::trt::InferenceMap_t* const trt_output_map,
                   GraphProto* const graph_proto,
...
TRTParam ConvertNnvmGraphToOnnx(
    const nnvm::Graph &g,
    std::unordered_map<std::string, NDArray> *const shared_buffer);
//    std::string graph_name = "TensorRT graph");
...
#endif  // MXNET_OPERATOR_CONTRIB_TRT_INL_H_
